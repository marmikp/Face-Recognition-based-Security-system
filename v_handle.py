...
def data_maintain(nam, im):
        ...
        r= requests.post(server_url+'/v_handle_data.php',data={'name': nam,'person':'Person Detected','date':dt,'time':time,'img_name':img_name})
        print(r.text)
        if r.text == "1":
                upload_img(now,im)
